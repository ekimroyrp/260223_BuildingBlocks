...
let fullAutoGeneratedShapes = [];
const fullAutoBuildState = {
  active: false,
  shapes: [],
  heights: [],
  shapeIndex: 0,
...
function sortParsedCellIndices(indices) {
  indices.sort((a, b) => {
    if (a.z !== b.z) return a.z - b.z;
    return a.x - b.x;
  });
...
function orderOutlineKeys(outlineSet) {
  if (!outlineSet || outlineSet.size === 0) return [];
  const unvisited = new Set(outlineSet);
  const start = sortParsedCellIndices(
    Array.from(unvisited, (key) => parseGroundCellKey(key))
  )[0];
  let currentKey = groundCellKey(start);
  let previousKey = null;
  const orderedKeys = [];
  while (unvisited.size > 0) {
    if (!unvisited.has(currentKey)) {
      const fallback = sortParsedCellIndices(
        Array.from(unvisited, (key) => parseGroundCellKey(key))
      )[0];
      currentKey = groundCellKey(fallback);
      previousKey = null;
    }
    ...
    const neighbors = getGroundNeighborKeys(currentKey, outlineSet).filter((key) => unvisited.has(key));
    let nextKey = null;
    if (neighbors.length > 0) {
      if (previousKey && neighbors.length > 1) {
        nextKey = neighbors.find((key) => key !== previousKey) || neighbors[0];
      } else {
        nextKey = neighbors[0];
      }
    } else {
      ...
      unvisited.forEach((candidateKey) => {
        const a = parseGroundCellKey(currentKey);
        const b = parseGroundCellKey(candidateKey);
...
function buildFullAutoShapeData(candidate) {
  if (!candidate || !candidate.cells || candidate.cells.size === 0) return null;
  const cells = sortParsedCellIndices(Array.from(candidate.cells, (key) => parseGroundCellKey(key)));
  if (cells.length === 0) return null;
  const outlineKeysOrdered = orderOutlineKeys(candidate.outline);
  const outline = outlineKeysOrdered.map((key) => parseGroundCellKey(key));
  const outlineSet = new Set(outlineKeysOrdered);
  const interior = cells.filter((index) => !outlineSet.has(groundCellKey(index)));
...
function buildFullAutoOutlinePattern() {
  ...
  for (let i = 0; i < entries.length; i += 1) {
    ...
    const shapeData = buildFullAutoShapeData(selected);
    if (!shapeData) continue;
    acceptedShapes.push(shapeData);
    selected.cells.forEach((key) => {
      addCellAndNeighborsToBlockedSet(key, blockedKeys);
    });
  }

  return acceptedShapes;
}

function getPaintedKeysFromFullAutoShapes(shapes) {
  const paintedKeys = new Set();
  if (!Array.isArray(shapes)) return paintedKeys;
  for (let i = 0; i < shapes.length; i += 1) {
    const shape = shapes[i];
    if (!shape || !Array.isArray(shape.cells)) continue;
    for (let c = 0; c < shape.cells.length; c += 1) {
      paintedKeys.add(groundCellKey(shape.cells[c]));
    }
  }
  return paintedKeys;
}
...
function stopFullAutomaticBuild(pushHistory = true) {
  if (pushHistory && fullAutoBuildState.changedBlocks) {
    pushHistoryState(snapshotState(), true);
  }
  fullAutoBuildState.active = false;
  fullAutoBuildState.shapes = [];
  fullAutoBuildState.heights = [];
  fullAutoBuildState.shapeIndex = 0;
  fullAutoBuildState.layerIndex = 0;
  fullAutoBuildState.outlineIndex = 0;
  fullAutoBuildState.lastStepTime = null;
  fullAutoBuildState.changedBlocks = false;
}

function runFullAutoGeneration() {
  if (!fullAutomaticMode) return;
  if (fullAutoBuildState.active) {
    stopFullAutomaticBuild(true);
  }
  fullAutoGeneratedShapes = buildFullAutoOutlinePattern();
  const paintedKeys = getPaintedKeysFromFullAutoShapes(fullAutoGeneratedShapes);
  stopSemiAutomaticBuild({ clearCompleted: true });
  applyGroundPaintPattern(paintedKeys);
}

function selectFullAutoShapeHeights(shapeCount) {
  const minStack = Math.max(1, Math.min(20, Math.round(Math.min(fullMinStack, fullMaxStack))));
  const maxStack = Math.max(1, Math.min(20, Math.round(Math.max(fullMinStack, fullMaxStack))));
  const rng = createSeededRng(Math.max(1, Math.round(fullStackSeed)));
  const heights = [];
  for (let i = 0; i < shapeCount; i += 1) {
    const h = minStack + Math.floor(rng() * (maxStack - minStack + 1));
    heights.push(h);
  }
  if (shapeCount > 1 && maxStack > minStack) {
    ...
    if (allSame) {
      ...
      for (let i = 0; i < heights.length; i += 1) {
        heights[i] = minStack + ((first - minStack + i) % range);
      }
    }
  }
  return heights;
...
function startFullAutomaticBuild() {
  if (!fullAutomaticMode) return;
  runFullAutoGeneration();
  if (!Array.isArray(fullAutoGeneratedShapes) || fullAutoGeneratedShapes.length === 0) {
    stopFullAutomaticBuild(false);
    return;
  }
  fullAutoBuildState.active = true;
  fullAutoBuildState.shapes = fullAutoGeneratedShapes;
  fullAutoBuildState.heights = selectFullAutoShapeHeights(fullAutoGeneratedShapes.length);
  fullAutoBuildState.shapeIndex = 0;
  fullAutoBuildState.layerIndex = 0;
  fullAutoBuildState.outlineIndex = 0;
  fullAutoBuildState.lastStepTime = null;
...
function updateFullAutomaticBuild(now) {
  if (!fullAutoBuildState.active || !Array.isArray(fullAutoBuildState.shapes)) return;
  const interval = Math.max(1, 1000 / Math.max(0.1, fullBuildRate));
  if (fullAutoBuildState.lastStepTime === null) {
    fullAutoBuildState.lastStepTime = now - interval;
  }
  if (now - fullAutoBuildState.lastStepTime < interval) return;

  const maxStepsPerFrame = 32;
  let steps = 0;

  while (steps < maxStepsPerFrame && now - fullAutoBuildState.lastStepTime >= interval) {
    const shape = fullAutoBuildState.shapes[fullAutoBuildState.shapeIndex];
    const targetHeight = fullAutoBuildState.heights[fullAutoBuildState.shapeIndex] || 1;
    if (!shape) {
      stopFullAutomaticBuild(true);
      break;
    }

    if (fullAutoBuildState.outlineIndex < shape.outline.length) {
      const cell = shape.outline[fullAutoBuildState.outlineIndex];
      const didAdd = addBlockAt(
        { x: cell.x, y: fullAutoBuildState.layerIndex, z: cell.z },
        currentColor,
        false
      );
      if (didAdd) {
        fullAutoBuildState.changedBlocks = true;
      }
      fullAutoBuildState.outlineIndex += 1;
    } else {
      fullAutoBuildState.outlineIndex = 0;
      fullAutoBuildState.layerIndex += 1;
      if (fullAutoBuildState.layerIndex >= targetHeight) {
        const topLayerY = Math.max(0, targetHeight - 1);
        for (let i = 0; i < shape.interior.length; i += 1) {
          const cell = shape.interior[i];
          const didFillAdd = addBlockAt({ x: cell.x, y: topLayerY, z: cell.z }, currentColor, false);
          if (didFillAdd) {
            fullAutoBuildState.changedBlocks = true;
          }
        }
        fullAutoBuildState.shapeIndex += 1;
        fullAutoBuildState.layerIndex = 0;
        fullAutoBuildState.outlineIndex = 0;
        if (fullAutoBuildState.shapeIndex >= fullAutoBuildState.shapes.length) {
          stopFullAutomaticBuild(true);
          break;
        }
      }
    ...
    fullAutoBuildState.lastStepTime += interval;
    steps += 1;
  }
}
...
function handlePaint() {
  ...
  if (fullAutoBuildState.active) return;
...
const fullAutoStartButton = document.getElementById('full-auto-start-button');
...
if (fullAutoStartButton) {
  fullAutoStartButton.addEventListener('click', () => {
    startFullAutomaticBuild();
  });
}
